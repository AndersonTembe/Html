$("document").ready(function () {
    function drawStuff() {
        ...
        var options = {
            title: 'Competitions per Olympics Edition',
            width: 800,
            ...
            axes: {
                x: {
                    0: { side: 'top', label: 'Competitions' } // Top x-axis.
                }
            },
            bar: { groupWidth: "90%" }
        };
        var composedUri = "http://192.168.160.58/Olympics/api/Statistics/Games_Competitions";
        ajaxHelper(composedUri, 'GET').done(function (stats) {
            data.addColumn('string', 'Olympics Edition');
            data.addColumn('number', 'Competitions');
